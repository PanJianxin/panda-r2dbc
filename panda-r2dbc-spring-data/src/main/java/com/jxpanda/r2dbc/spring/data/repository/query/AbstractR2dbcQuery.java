...
public abstract class AbstractR2dbcQuery implements RepositoryQuery {
	...
	private Publisher<?> executeQuery(RelationalParameterAccessor parameterAccessor, PreparedOperation<?> operation) {
		...
		R2dbcQueryExecution execution = new R2dbcQueryExecution.ResultProcessingExecution(getExecutionToWrap(processor.getReturnedType()),
				new R2dbcQueryExecution.ResultProcessingConverter(processor, converter.getMappingContext(), instantiators));
