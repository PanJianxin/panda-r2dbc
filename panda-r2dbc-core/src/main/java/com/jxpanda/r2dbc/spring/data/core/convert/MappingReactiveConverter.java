...
public class MappingReactiveConverter extends MappingR2dbcConverter {
    ...
    private record RelationalPropertyValueProviderDecorator(DocumentValueProvider originalValueProvider,
        ...
        public <T> T getPropertyValue(RelationalPersistentProperty property) {
            boolean hasTypeHandler = typeHandlers.hasTypeHandler(property);
            boolean hasAlias = hasAlias(property);
            Object value = hasAlias || hasTypeHandler ? getValueWithAlias(property) : originalValueProvider.getPropertyValue(property);
            // 检查是否有针对当前属性的类型处理器
            if (hasTypeHandler) {
                ...
                // 如果有别名的话，尝试从别名获取值
                value = hasAlias ? value : documentAccessor.get(property);
