...
public record R2dbcConfigProperties(Database database, Mapping mapping, LogicDelete logicDelete) {
     ...
     */
    public record Database(boolean forceQuote, IdStrategy idStrategy, int dataCenterId, int workerId) {

        public static Database empty() {
            return new Database(true, IdStrategy.DEFAULT, 0, 0);
        }
     ...
     */
    public record Mapping(NamingStrategy namingStrategy, ValidationStrategy validationStrategy) {

        public static Mapping empty() {
            return new Mapping(NamingStrategy.DEFAULT, ValidationStrategy.DEFAULT);
        }
     ...
     */
    public record LogicDelete(boolean enable, String field, Value undeleteValue, Value deleteValue) {
