...
import com.jxpanda.r2dbc.spring.data.core.operation.contract.R2dbcInsertOperation;
...
public final class R2dbcInsertOperationSupport extends R2dbcOperationSupport implements R2dbcInsertOperation {
    ...
    @NonNull
    @Override
    public <T> R2dbcInsert<T> insert(@NonNull Class<T> entityType) {

        Assert.notNull(entityType, "entityType must not be null");

        return new R2dbcInsertSupport<>(R2dbcOperationContext.<T, T>builder()
                .template(template)
    ...
    private static final class R2dbcInsertSupport<T> extends R2dbcSupport<T> implements R2dbcInsert<T> {
        private R2dbcInsertSupport(R2dbcOperationContext<T, T> operationContext) {
        ...
        public ReactiveInsertOperation.TerminatingInsert<T> into(@NonNull SqlIdentifier tableName) {
            ...
            return newSupport(rebuilder().tableName(tableName), R2dbcInsertSupport::new);
        }
