...
import com.jxpanda.r2dbc.spring.data.core.operation.contract.R2dbcDestroyOperation;
...
public final class R2dbcDestroyOperationSupport extends R2dbcOperationSupport implements R2dbcDestroyOperation {
    ...
    @Override
    public <T> R2dbcDestroy<T> destroy(Class<T> entityType) {
        return new R2dbcDestroySupport<>(R2dbcOperationContext.<T, T>builder()
                .template(template)
    ...
    private static final class R2dbcDestroySupport<T> extends R2dbcSupport<T> implements R2dbcDestroyOperation.R2dbcDestroy<T> {
        ...
        private R2dbcDestroySupport(R2dbcOperationContext<T, T> operationContext) {
        ...
        public ReactiveDeleteOperation.DeleteWithQuery from(@NonNull SqlIdentifier tableName) {
            ...
            return newSupport(rebuilder().tableName(tableName), R2dbcDestroySupport::new);
        }
        ...
        public ReactiveDeleteOperation.TerminatingDelete matching(@NonNull Query query) {
            ...
            return newSupport(rebuilder().query(query), R2dbcDestroySupport::new);
        }
