...
import com.jxpanda.r2dbc.spring.data.core.operation.contract.R2dbcDeleteOperation;
...
public final class R2dbcDeleteOperationSupport extends R2dbcOperationSupport implements R2dbcDeleteOperation {
    ...
    @NonNull
    @Override
    public <T> R2dbcDeleteOperation.R2dbcDelete<T> delete(@NonNull Class<T> entityType) {

        Assert.notNull(entityType, "entityType must not be null");

        return new R2dbcDeleteSupport<>(R2dbcOperationContext.<T, T>builder()
                .template(template)
    ...
    private static final class R2dbcDeleteSupport<T> extends R2dbcSupport<T> implements R2dbcDeleteOperation.R2dbcDelete<T> {
        ...
        private R2dbcDeleteSupport(R2dbcOperationContext<T, T> operationContext) {
...
//            return new R2dbcDeleteSupport<>(this.template, this.entityType, this.query, tableName);
            return newSupport(rebuilder().tableName(tableName), R2dbcDeleteSupport::new);
        }
...
//            return new R2dbcDeleteSupport<>(this.template, this.entityType, query, this.tableName);
            return newSupport(rebuilder().query(query), R2dbcDeleteSupport::new);
        }
        ...
        public <ID> Mono<Boolean> byId(ID id) {
            ...
            return executorBuilder(R2dbcDeleteExecutor::<T, Long>builder)
                    .resultType(Long.class)
                    .queryHandler(operationContext -> QueryKit.queryById(operationContext.getEntityType(), id))
                    .build()
                    .execute()
                    .map(it -> it > 0);
        }
        ...
        public <ID> Mono<Long> byIds(Collection<ID> ids) {
            ...
            return executorBuilder(R2dbcDeleteExecutor::<T, Long>builder)
                    .resultType(Long.class)
                    .queryHandler(operationContext -> QueryKit.queryById(operationContext.getEntityType(), ids))
                    .build()
                    .execute();
        }
