...
import com.jxpanda.r2dbc.spring.data.core.enhance.BitmaskEnum;
...
public enum Rule implements RuleFunction {
    ...
    LEFT_LIKE((criteriaStep, param) -> criteriaStep.like(StringConstant.PERCENT_SIGN + param)),
    RIGHT_LIKE((criteriaStep, param) -> criteriaStep.like(param + StringConstant.PERCENT_SIGN)),
    ...
    MATCHES_ALL_BIT((criteriaStep, params) -> {
        List<Object> objects = ReflectionKit.cast(params);
        return criteriaStep.matchesAllBit(BitmaskEnum.toMask(objects));
    }),
    MATCHES_ANY_BIT((criteriaStep, params) -> {
        List<Object> objects = ReflectionKit.cast(params);
        return criteriaStep.matchesAnyBit(BitmaskEnum.toMask(objects));
    });
