...
* `R2dbcEntityTemplate` as central class for standardEntity-bound operations that increases productivity when performing common R2DBC operations with integrated object mapping between rows and POJOs.
...
+ You can let Spring auto-discover your `R2dbcDialect` by registering a class that implements `DialectResolver$R2dbcDialectProvider` through `META-INF/spring.factories`.
