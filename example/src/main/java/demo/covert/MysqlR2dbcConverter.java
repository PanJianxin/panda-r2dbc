...
public class MysqlR2dbcConverter extends MappingR2dbcConverter {

    public MysqlR2dbcConverter(
    ...
    @Override
    @NonNull
    public <R> R read(@NonNull Class<R> type, @NonNull Row row) {
    ...
    @Override
    @NonNull
    public <R> R read(@NonNull Class<R> type, @NonNull Row row, RowMetadata metadata) {
        ...
        if (getConversions().hasCustomReadTarget(Row.class, rawType)
            ...
            R convert = getConversionService().convert(row, rawType);
            return convert == null ? JsonKit.fromJson("{}", new TypeReference<>() {
                @Override
                public Type getType() {
                    return rawType;
                }
        ...
        return read((RelationalPersistentEntity<R>) getMappingContext().getRequiredPersistentEntity(typeInfo), row, metadata);
    }
    ...
    private <R> R read(RelationalPersistentEntity<R> entity, Row row, @Nullable RowMetadata metadata) {
        ...
        if (entity.requiresPropertyPopulation()) {
            ...
            for (RelationalPersistentProperty property : entity) {
                ...
                TableColumn annotation = property.getField().getAnnotation(TableColumn.class);
    ...
    private Object readFrom(Row row, @Nullable RowMetadata metadata, RelationalPersistentProperty property,
        ...
        try {
            ...
            if (annotation != null && annotation.isJson()) {
                return JsonKit.fromJson(value.toString(), new TypeReference<>() {
                    @Override
                    public Type getType() {
    ...
    private Object getPotentiallyConvertedSimpleRead(@Nullable Object value, @Nullable Class<?> target) {
        ...
        if (Enum.class.isAssignableFrom(target)) {
            return EnumTypeHandler.translate(target, value.toString());
        }
    ...
    private Object extractGeneratedIdentifier(Row row, RowMetadata metadata, String idColumnName) {
        ...
        if (RowMetadataKit.containsColumn(metadata, idColumnName)) {
            return row.get(idColumnName);
        }

        Iterable<? extends ColumnMetadata> columns = RowMetadataKit.getColumnMetadata(metadata);
    ...
    enum NoOpParameterValueProvider implements ParameterValueProvider<RelationalPersistentProperty> {
        ...
        @Override
        public <T> T getParameterValue(org.springframework.data.mapping.Parameter<T, RelationalPersistentProperty> parameter) {
            return null;
        }
    }
